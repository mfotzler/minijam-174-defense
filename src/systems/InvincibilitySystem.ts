...
export default class InvincibilitySystem implements  System {
	constructor(private world: World, private scene: BaseScene) {
	...
	private onMakeInvincible(entity: EntityDefinition<BugComponents>) {
		entity.invincibility.currentDuration = entity.invincibility.maxDuration;
		this.flash(entity);
	}
	...
	step(data: StepData) {
			const currentlyInvincible = this.world.entityProvider.entities.filter(InvincibilitySystem.isInvincible);
